...
int main(){
	...
	*/
	...
	for(int i=0;i<n;i++) disk[i]=new string[len];
	exist[l1]=1;
	for(int i=0;i<len;i++) disk[l1][i]=l2.substr(8*i,8);
	for(int i=1;i<l;i++){
	    cin>>l1>>l2;
	    exist[l1]=1;
	    for(int j=0;j<len;j++) disk[l1][j]=l2.substr(8*j,8);
	}
	cin>>m;
		...
		if(exist[t]==1){
			...
			if(l==n-1){
				if(f<len){
					ans[i]="00000000";
					for(int j=0;j<n;j++){
						if(j!=t) ans[i]=xor1(ans[i],disk[j][f]);
					} 
				}
				else ans[i]="-";
			}
